                    ...
                    <div class="row g-4">
                        @foreach($packages as $package)
                            <div class="col-md-4 col-sm-6">
                                <div class="token-package-card position-relative" style="
                                ...
                                onclick="selectPackage({{ $package['id'] }})">
                                    ...
                                    <div class="price-container">
                                        ...
                                        @endif
                                        <h3 class="fw-bold mb-1">Rp {{ number_format($package['price'], 0, ',', '.') }}</h3>
                                        @if($package['discount'] > 0)
                                    ...
                                    <div class="token-price-info mt-3 mb-2">
                                        <span class="small text-muted">{{ $package['price_per_token'] }} per token</span>
                                    </div>

                                    <div class="mt-3">
                                        <input type="radio" name="package_id" value="{{ $package['id'] }}" id="package_{{ $package['id'] }}"
                                            style="transform: scale(1.5); margin-right: 8px; accent-color: var(--primary);"
                                            {{ $selectedPackage == $package['id'] ? 'checked' : '' }}>
                                        <label for="package_{{ $package['id'] }}" class="form-check-label">Select</label>
                                    </div>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="neo-card mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('tokens.process-purchase') }}" method="POST" id="paymentForm">
                        @csrf
                        <input type="hidden" name="package_id" id="selected_package_id" value="{{ $selectedPackage }}">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        " onclick="selectPaymentMethod('bank_transfer')">
                                            ...
                                            <input type="radio" name="payment_method" value="bank_transfer" id="payment_bank_transfer"
                                                {{ $paymentMethod == 'bank_transfer' ? 'checked' : '' }} style="display:none;">
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="payment-method-card" style="
                                        ...
                                        " onclick="selectPaymentMethod('credit_card')">
                                            ...
                                            <input type="radio" name="payment_method" value="credit_card" id="payment_credit_card"
                                                {{ $paymentMethod == 'credit_card' ? 'checked' : '' }} style="display:none;">
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="payment-method-card" style="
                                        ...
                                        " onclick="selectPaymentMethod('e_wallet')">
                                            ...
                                            <input type="radio" name="payment_method" value="e_wallet" id="payment_e_wallet"
                                                {{ $paymentMethod == 'e_wallet' ? 'checked' : '' }} style="display:none;">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div id="bank_transfer_form" class="payment-details-form {{ $paymentMethod == 'bank_transfer' ? '' : 'd-none' }}">
                            <div class="alert alert-info" style="border: 3px solid #212529; border-radius: 8px; box-shadow: 5px 5px 0 rgba(0,0,0,0.2);">
                                <i class="fas fa-info-circle me-2"></i> After submitting, you will receive bank transfer details.
                            </div>
                        ...
                        <div id="credit_card_form" class="payment-details-form {{ $paymentMethod == 'credit_card' ? '' : 'd-none' }}">
                            ...
                            <x-form-input name="card_holder" label="Cardholder Name" placeholder="Enter name on card" />
                        </div>

                        <div id="e_wallet_form" class="payment-details-form {{ $paymentMethod == 'e_wallet' ? '' : 'd-none' }}">
                            <x-form-select
                        ...
                        <div class="mt-4 pt-3 border-top">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <span class="fw-bold" id="selected_package_display">{{ $selectedPackageAmount ?? '0' }} Tokens</span>
                            </div>

                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <span>Price:</span>
                                <span class="fw-bold" id="selected_price_display">Rp {{ number_format($selectedPackagePrice ?? 0, 0, ',', '.') }}</span>
                            </div>
            ...
            <div class="neo-card mb-4">
                ...
                <div class="card-body text-center">
                    ...
                    <div class="after-purchase mt-3">
                        ...
                        <h3 class="fw-bold" id="balance_after_purchase">{{ $currentBalance + ($selectedPackageAmount ?? 0) }} Tokens</h3>
                    </div>
                </div>
            </div>
...
<script>
    function selectPackage(packageId) {
        ...
        if (selectedCard) {
            ...
            // Note: In a real implementation, you'd fetch these values from your packages data
            ...
            if (selectedPackage) {
                ...
                document.getElementById('balance_after_purchase').innerText =
                    `${currentBalance + selectedPackage.amount} Tokens`;
            }
        }
    }
    ...
    function selectPaymentMethod(method) {
        ...
        document.getElementById(`${method}_form`).classList.remove('d-none');
        ...
        document.querySelector(`.payment-method-card:has(#payment_${method})`).style.backgroundColor = '#f8f9fa';
        document.querySelector(`.payment-method-card:has(#payment_${method})`).style.boxShadow = '5px 5px 0 var(--primary)';
